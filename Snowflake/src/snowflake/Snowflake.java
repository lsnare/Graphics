...
public class Snowflake extends SkelGL {
	...
	public KochSet computeKochSet(Vector2f s, Vector2f d) {
		...
		k.o = new Vector2f(s.x + (d.x - s.x) * 2 / 3.0f, s.y + (d.y - s.y) * 2
				/ 3.0f);
		float px = (float) (Math.cos(deg60) * (k.o.x - k.m.x) - Math.sin(deg60)
				* (k.o.y - k.m.y) + k.m.x);
		float py = (float) (Math.sin(deg60) * (k.o.x - k.m.x) + Math.cos(deg60)
				* (k.o.y - k.m.y) + k.m.y);
	...
	public void initSnowFlake() {
		Vector2f[] vertices = { new Vector2f(-0.6f, -0.6f),
				new Vector2f(0f, 0.6f), new Vector2f(0.6f, -0.6f), };
		index = 0;
	...
	protected void renderGL() {
		...
		//if the index is not a multiple of 6, we will have a strobing effect
		for (int i = 0; i < 6; i++) {
			GL11.glColor3f(palette[colIndex%6].x, palette[colIndex%6].y, palette[colIndex%6].z);
			colIndex++;
			renderSnowFlake();
